...
public class StudyImporterForBatPlant extends StudyImporterWithListener {
    private static final Log LOG = LogFactory.getLog(StudyImporterForBatPlant.class);
    ...
    public static void appendReferenceToMap(JsonNode aReference, Map<String, Map<String, String>> refMap) {
        if (hasNonEmptyValueFor(aReference, "id")) {
            ...
            if (hasNonEmptyValueFor(aReference, "doi")) {
                props.put(StudyImporterForTSV.REFERENCE_DOI, aReference.get("doi").asText());
            }
            if (hasNonEmptyValueFor(aReference, "bibtex")) {
                props.put(StudyImporterForTSV.REFERENCE_CITATION, aReference.get("bibtex").asText());
            } else {
                if (hasNonEmptyValueFor(aReference, "first_author")
                        && hasNonEmptyValueFor(aReference, "year"))
                    props.put(StudyImporterForTSV.REFERENCE_CITATION, aReference.get("first_author").asText() + " " + aReference.get("year").asText());
            }
        ...
        if (hasNonEmptyValueFor(singleNode, "geom")) {
            ...
            if (hasNonEmptyValueFor(geom, "type") && hasNonEmptyValueFor(geom, "coordinates")) {
                ...
                if (coordinates.isArray() && coordinates.size() == 2) {
                    props.put(StudyImporterForTSV.DECIMAL_LONGITUDE, coordinates.get(0).asText());
                    props.put(StudyImporterForTSV.DECIMAL_LATITUDE, coordinates.get(1).asText());
                }
            }
        }
    ...
    public static Map<String, String> parseInteraction(JsonNode singleInteraction, Map<String, Map<String, String>> nodeMap, Map<String, Map<String, String>> networkMap) throws StudyImporterException {
        ...
        interaction.put(StudyImporterForTSV.BASIS_OF_RECORD_NAME, basisOfRecord);
