...
public class CypherQueryBuilderTest {
    ...
    public static final String EXPECTED_ACCORDING_TO_START_CLAUSE = "CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) OR (has(study.citation) AND study.citation =~ {accordingTo}) OR (has(study.source) AND study.source =~ {accordingTo}) WITH study ";
    ...
    private static String expectedMatchClause(String expectedInteractionClause, boolean hasSpatialConstraints, boolean requestedSpatialInfo, boolean refutes) {
        ...
        return "MATCH sourceTaxon<-[:CLASSIFIED_AS]-" + expectedInteractionClause + "-[:CLASSIFIED_AS]->targetTaxon, sourceSpecimen<-[collected_rel:" + studyRel + "]-study" + spatialClause;
    }
    ...
    public void findInteractionForSourceAndTargetTaxaLocations() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) " +
    ...
    public void findInteractionTypesForTaxon() {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START taxon = node:taxonPaths({taxon_name}) MATCH taxon-[rel:" + concatInteractionTypes + "]->otherTaxon RETURN distinct(type(rel)) as interaction_type"));
    ...
    protected void assertFindInteractionsAccordingToWithTaxa(Map<String, String[]> fieldParams) {
        ...
        assertThat(query.getQuery(), is(EXPECTED_ACCORDING_TO_START_CLAUSE +
                ...
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i).*(\\\\Qinaturalist\\\\E).*, source_taxon_name=path:\\\"Arthropoda\\\", target_taxon_name=path:\\\"Arthropoda\\\"}")));
    }
    ...
    public void findInteractionsAccordingToWithTargetTaxaOnly() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT);
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) OR (has(study.citation) AND study.citation =~ {accordingTo}) OR (has(study.source) AND study.source =~ {accordingTo}) WITH study " +
                EXPECTED_MATCH_CLAUSE_DISTINCT +
                "WHERE " + hasTargetTaxon("Arthropoda") +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i).*(\\\\Qinaturalist\\\\E).*, target_taxon_name=path:\\\"Arthropoda\\\"}")));
    }
    ...
    public void findInteractionsAccordingToWithTargetTaxaOnly2() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT);
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) WITH study " +
                EXPECTED_MATCH_CLAUSE_DISTINCT +
                "WHERE " + hasTargetTaxon("Arthropoda") +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i)(\\\\Qhttp://inaturalist.org/bla\\\\E), target_taxon_name=path:\\\"Arthropoda\\\"}")));
    }
    ...
    public void findInteractionsDOIAccordingToWithTargetTaxaOnly2() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT);
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.doi) AND study.doi =~ {accordingTo}) WITH study " +
                EXPECTED_MATCH_CLAUSE_DISTINCT +
                "WHERE " + hasTargetTaxon("Arthropoda") +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
    ...
    public void accordingToDataset() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT);
        assertThat(query.getQuery(), is("CYPHER 1.9 START dataset = node:datasets({accordingTo}) " +
    ...
    public void findNumberOfStudiesForDistinctInteractionsAccordingTo() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT);
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') " +
        ...
        Map<String, String> expected = new HashMap<String, String>() {{
            put("target_taxon_name", "path:\\\"Arthropoda\\\"");
            put("accordingTo", "(?i).*(\\\\QsomeSource\\\\E).*");
        }};
        assertThat(query.getParams(), is(expected));
    }
    ...
    public void findInteractionsAccordingToWithTargetTaxaOnly3() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) WITH study " +
                ...
                "WHERE " + hasTargetTaxon("Arthropoda") +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i)(\\\\Qhttp://inaturalist.org/bla\\\\E|\\\\Qhttp://inaturalist.org/bla2\\\\E), target_taxon_name=path:\\\"Arthropoda\\\"}")));
    }
    ...
    @Test
    public void findRefutingAccordingTo() throws IOException {
        HashMap<String, String[]> params = new HashMap<String, String[]>() {
            {
                put("accordingTo", new String[]{"inaturalist"});
    ...
    public void findInteractionsAccordingToWithSourceTaxaOnlyAndExactMatchOnly() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) OR (has(study.citation) AND study.citation =~ {accordingTo}) OR (has(study.source) AND study.source =~ {accordingTo}) WITH study " +
                ...
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i).*(\\\\Qinaturalist\\\\E).*, source_taxon_name=name:\\\"Arthropoda\\\"}")));
    }
    ...
    public void findInteractionsAccordingToWithSourceTaxaOnlyAndExactMatchOnlyIncludeObservations() throws IOException {
        ...
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_ALL);
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxons({source_taxon_name}) " +
    ...
    public void findInteractionsAccordingToNoTaxa() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) OR (has(study.citation) AND study.citation =~ {accordingTo}) OR (has(study.source) AND study.source =~ {accordingTo}) WITH study " +
                EXPECTED_MATCH_CLAUSE_DISTINCT +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i).*(\\\\Qinaturalist\\\\E).*}")));
    }
    ...
    public void findInteractionsAccordingToMultipleNoTaxa() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START study = node:studies('*:*') WHERE (has(study.externalId) AND study.externalId =~ {accordingTo}) OR (has(study.citation) AND study.citation =~ {accordingTo}) OR (has(study.source) AND study.source =~ {accordingTo}) WITH study " +
                EXPECTED_MATCH_CLAUSE_DISTINCT +
                "WITH distinct targetTaxon, interaction.label as iType, sourceTaxon RETURN sourceTaxon.name as source_taxon_name,targetTaxon.name as target_taxon_name"));
        assertThat(query.getParams().toString(), is(is("{accordingTo=(?i).*(\\\\Qinaturalist\\\\E|\\\\Qgomexsi.edu\\\\E).*}")));
    }
    ...
    public void findTaxaAtLocationsDistinct() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START loc = node:locations('latitude:*') " +
    ...
    public void findTaxaAtLocationsDistinctNoSpatialParam() throws IOException {
        ...
        CypherQuery query = CypherQueryBuilder.createDistinctTaxaInLocationQuery(params);
        assertThat(query.getQuery(), is("CYPHER 1.9 START taxon = node:taxons('*:*') " +
                "RETURN distinct(taxon.name?) as taxon_name, taxon.commonNames? as taxon_common_names, taxon.externalId? as taxon_external_id, taxon.path? as taxon_path, taxon.pathIds? as taxon_path_ids, taxon.pathNames? as taxon_path_ranks"));
        assertThat(query.getParams().isEmpty(), is(true));
    }
    ...
    public void findTaxaAtLocationsDistinctNoSpatialInteractTypesParam() throws IOException {
        ...
        CypherQuery query = CypherQueryBuilder.createDistinctTaxaInLocationQuery(params);
        assertThat(query.getQuery(), is("CYPHER 1.9 START taxon = node:taxons('*:*') MATCH taxon-[:" + InteractUtil.interactionsCypherClause(PREYS_UPON, PARASITE_OF) + "]->otherTaxon RETURN distinct(taxon.name?) as taxon_name, taxon.commonNames? as taxon_common_names, taxon.externalId? as taxon_external_id, taxon.path? as taxon_path, taxon.pathIds? as taxon_path_ids, taxon.pathNames? as taxon_path_ranks"));
        assertThat(query.getParams().isEmpty(), is(true));
    }
    ...
    public void findInteractions() throws IOException {
        ...
        String expectedQuery = "CYPHER 1.9 START sourceTaxon = node:taxons('*:*') MATCH sourceTaxon-[interaction:INTERACTS_WITH|PREYS_UPON|PARASITE_OF|HAS_HOST|HOST_OF|POLLINATES|PERCHING_ON|ATE|SYMBIONT_OF|PREYED_UPON_BY|POLLINATED_BY|EATEN_BY|HAS_PARASITE|PERCHED_ON_BY|HAS_PATHOGEN|PATHOGEN_OF|HAS_VECTOR|VECTOR_OF|VISITED_BY|VISITS|FLOWERS_VISITED_BY|VISITS_FLOWERS_OF|INHABITED_BY|INHABITS|CREATES_HABITAT_FOR|IS_HABITAT_OF|LIVED_ON_BY|LIVES_ON|LIVED_INSIDE_OF_BY|LIVES_INSIDE_OF|LIVED_NEAR_BY|LIVES_NEAR|LIVED_UNDER_BY|LIVES_UNDER|LIVES_WITH|ENDOPARASITE_OF|HAS_ENDOPARASITE|HYPERPARASITE_OF|HAS_HYPERPARASITE|HYPERPARASITOID_OF|HAS_HYPERPARASITOID|ECTOPARASITE_OF|HAS_ECTOPARASITE|KLEPTOPARASITE_OF|HAS_KLEPTOPARASITE|PARASITOID_OF|HAS_PARASITOID|ENDOPARASITOID_OF|HAS_ENDOPARASITOID|ECTOPARASITOID_OF|HAS_ECTOPARASITOID|GUEST_OF|HAS_GUEST_OF|FARMED_BY|FARMS|DAMAGED_BY|DAMAGES|DISPERSAL_VECTOR_OF|HAS_DISPERAL_VECTOR|KILLED_BY|KILLS|EPIPHITE_OF|HAS_EPIPHITE|LAYS_EGGS_ON|HAS_EGGS_LAYED_ON_BY|COMMENSALIST_OF]->targetTaxon RETURN sourceTaxon.name as source_taxon_name,interaction.label? as interaction_type,targetTaxon.name as target_taxon_name";
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT_BY_NAME_ONLY);
        assertThat(query.getQuery(), is(expectedQuery));
    ...
    public void findInteractionForTargetTaxaOnlyNoLocation() throws IOException {
        ...
        String expectedQuery = "CYPHER 1.9 START targetTaxon = node:taxonPaths({target_taxon_name}) " +
                EXPECTED_MATCH_CLAUSE_ALL +
                EXPECTED_RETURN_CLAUSE;
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_ALL);
        assertThat(query.getQuery(), is(expectedQuery));
    ...
    public void findInteractionForAnimaliaAndAnimalia() throws IOException {
        ...
        String expectedQuery = "CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) MATCH sourceTaxon-[interaction:INTERACTS_WITH|PREYS_UPON|PARASITE_OF|HAS_HOST|HOST_OF|POLLINATES|PERCHING_ON|ATE|SYMBIONT_OF|PREYED_UPON_BY|POLLINATED_BY|EATEN_BY|HAS_PARASITE|PERCHED_ON_BY|HAS_PATHOGEN|PATHOGEN_OF|HAS_VECTOR|VECTOR_OF|VISITED_BY|VISITS|FLOWERS_VISITED_BY|VISITS_FLOWERS_OF|INHABITED_BY|INHABITS|CREATES_HABITAT_FOR|IS_HABITAT_OF|LIVED_ON_BY|LIVES_ON|LIVED_INSIDE_OF_BY|LIVES_INSIDE_OF|LIVED_NEAR_BY|LIVES_NEAR|LIVED_UNDER_BY|LIVES_UNDER|LIVES_WITH|ENDOPARASITE_OF|HAS_ENDOPARASITE|HYPERPARASITE_OF|HAS_HYPERPARASITE|HYPERPARASITOID_OF|HAS_HYPERPARASITOID|ECTOPARASITE_OF|HAS_ECTOPARASITE|KLEPTOPARASITE_OF|HAS_KLEPTOPARASITE|PARASITOID_OF|HAS_PARASITOID|ENDOPARASITOID_OF|HAS_ENDOPARASITOID|ECTOPARASITOID_OF|HAS_ECTOPARASITOID|GUEST_OF|HAS_GUEST_OF|FARMED_BY|FARMS|DAMAGED_BY|DAMAGES|DISPERSAL_VECTOR_OF|HAS_DISPERAL_VECTOR|KILLED_BY|KILLS|EPIPHITE_OF|HAS_EPIPHITE|LAYS_EGGS_ON|HAS_EGGS_LAYED_ON_BY|COMMENSALIST_OF]->targetTaxon WHERE (has(targetTaxon.externalIds) AND targetTaxon.externalIds =~ '(.*(Animalia).*)') RETURN sourceTaxon.name as source_taxon_name,sourceTaxon.externalId? as source_taxon_external_id,targetTaxon.name as target_taxon_name,targetTaxon.externalId? as target_taxon_external_id,interaction.label? as interaction_type,interaction.count as number_of_interactions";
        CypherQuery query = buildInteractionQuery(params, MULTI_TAXON_DISTINCT_BY_NAME_ONLY);
        assertThat(query.getQuery(), is(expectedQuery));
    ...
    public void findInteractionNoParams() throws IOException {
        String expectedQuery = "CYPHER 1.9 START study = node:studies('*:*') " +
                EXPECTED_MATCH_CLAUSE_ALL +
                EXPECTED_RETURN_CLAUSE;
    ...
    public void findPreysOnWithLocation() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) MATCH sourceTaxon<-[:CLASSIFIED_AS]-sourceSpecimen-[interaction:PREYS_UPON]->targetSpecimen-[:CLASSIFIED_AS]->targetTaxon, sourceSpecimen<-[collected_rel:COLLECTED]-study, sourceSpecimen-[:COLLECTED_AT]->loc WHERE has(loc.latitude) AND has(loc.longitude) AND loc.latitude < 23.32 AND loc.longitude > -67.87 AND loc.latitude > 12.79 AND loc.longitude < -57.08 AND " + HAS_TARGET_TAXON_PLANTAE + expectedReturnClause()));
    ...
    public void findPlantPreyObservationsWithoutLocation() throws IOException {
        HashMap<String, String[]> params = new HashMap<String, String[]>();
        CypherQuery query = buildInteractionQuery("Homo sapiens", "preysOn", "Plantae", params, SINGLE_TAXON_ALL);
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) MATCH sourceTaxon<-[:CLASSIFIED_AS]-sourceSpecimen-[interaction:" + InteractUtil.interactionsCypherClause(PREYS_UPON) + "]->targetSpecimen-[:CLASSIFIED_AS]->targetTaxon, sourceSpecimen<-[collected_rel:COLLECTED]-study, sourceSpecimen-[?:COLLECTED_AT]->loc WHERE " + HAS_TARGET_TAXON_PLANTAE + expectedReturnClause()));
        assertThat(query.getParams().toString(), is("{source_taxon_name=path:\\\"Homo sapiens\\\", target_taxon_name=path:\\\"Plantae\\\"}"));
    }
    ...
    public void findPlantParasiteObservationsWithoutLocation() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) MATCH sourceTaxon<-[:CLASSIFIED_AS]-sourceSpecimen-[interaction:" + InteractUtil.interactionsCypherClause(PARASITE_OF) + "]->targetSpecimen-[:CLASSIFIED_AS]->targetTaxon, sourceSpecimen<-[collected_rel:COLLECTED]-study, sourceSpecimen-[?:COLLECTED_AT]->loc WHERE " + HAS_TARGET_TAXON_PLANTAE + expectedReturnClause()));
        assertThat(query.getParams().toString(), is("{source_taxon_name=path:\\\"Homo sapiens\\\", target_taxon_name=path:\\\"Plantae\\\"}"));
    }
    ...
    public void findKillsObservationsNoLocation() throws IOException {
        ...
        assertThat(query.getQuery(), is("CYPHER 1.9 START sourceTaxon = node:taxonPaths({source_taxon_name}) MATCH sourceTaxon<-[:CLASSIFIED_AS]-sourceSpecimen-[interaction:" + InteractUtil.interactionsCypherClause(KILLS) + "]->targetSpecimen-[:CLASSIFIED_AS]->targetTaxon, sourceSpecimen<-[collected_rel:COLLECTED]-study, sourceSpecimen-[?:COLLECTED_AT]->loc " + expectedReturnClause()));
        assertThat(query.getParams().toString(), is("{source_taxon_name=path:\\\"Homo sapiens\\\"}"));
    }
    ...
    public void locationsNoConstraints() {
        assertThat(locations().getQuery(), is("CYPHER 1.9 START loc = node:locations('latitude:*') RETURN loc.latitude? as latitude, loc.longitude? as longitude, loc.footprintWKT? as footprintWKT"));
    }
    ...
    public void locationsAccordingTo() {
        ...
        assertThat(locationsQuery.getParams().toString(), is("{accordingTo=(?i).*(\\\\Qsome source\\\\E).*}"));
    }
    ...
    public void regexAccordingToGoMexSI() {
        ...
        assertThat(regex, is("(?i)(\\\\Qhttp://gomexsi.tamucc.edu\\\\E|\\\\Qhttp://gomexsi.tamucc.edu.\\\\E)"));

        regex = CypherQueryBuilder.regexForAccordingTo(Arrays.asList("http://gomexsi.tamucc.edu", "https://example.com"));
        assertThat(regex, is("(?i)(\\\\Qhttp://gomexsi.tamucc.edu\\\\E|\\\\Qhttps://example.com\\\\E|\\\\Qhttp://gomexsi.tamucc.edu.\\\\E)"));

        regex = CypherQueryBuilder.regexForAccordingTo(Arrays.asList("https://example.com"));
        assertThat(regex, is("(?i)(\\\\Qhttps://example.com\\\\E)"));
    }
