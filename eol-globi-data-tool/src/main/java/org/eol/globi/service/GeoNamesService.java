...
public class GeoNamesService {
    ...
    private Map<String, Term> GEO_NAMES_MAP = new HashMap<String, Term>() {{
    ...
    public LatLng findLatLngForSPIRELocality(String spireLocality) throws IOException {
        LatLng point = pointCache.get(spireLocality);
    ...
    private LatLng retrievePointForLocality(String spireLocality) throws IOException {
        LatLng point = null;
        Term term = GEO_NAMES_MAP.get(spireLocality);
        if (term != null) {
            String idString = term.getId();
            if (GEO_NAMES_MAP.containsKey(spireLocality) && idString.startsWith(GEONAMES.getIdPrefix())) {
